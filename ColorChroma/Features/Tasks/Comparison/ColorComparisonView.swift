// Features/Comparison/ColorComparisonView.swift

import SwiftUI
import PhotosUI

struct ColorComparisonView: View {
    @StateObject private var service = ComparisonService()
    ...
    private var showingResults: Bool {
        comparisonResult != nil || errorMessage != nil
    }
    ...
    var body: some View {
            ...
            VStack(spacing: 20) {
                ...
                VStack(spacing: 20) {
                    if showingResults {
                        ...
                        CompactImageView(title: "Item 1", image: imageA)
                        CompactImageView(title: "Item 2", image: imageB)
                    } else {
                        // Show full image selectors when not displaying results
                        ImageSelector(title: "Item 1", selectedItem: $selectedItemA, image: $imageA)
                        ImageSelector(title: "Item 2", selectedItem: $selectedItemB, image: $imageB)
                    }
                }
                ...
                if !showingResults {
                    ...
                    Button(action: performComparison) {
                        Label("Compare Colors", systemImage: "arrow.left.arrow.right.circle.fill")
                            .font(.headline)
                            .padding()
                ...
                // Analysis progress
                if isAnalyzing {
                    ProgressView("Analyzing comparison...")
                        .padding(.top, 20)
                }
                
                // Results section
                if let comparisonResult {
                    ResultView(text: comparisonResult, onReset: reset)
        ...
        .navigationTitle("Compare Colors")
    ...
    private func performComparison() {
        ...
        Task {
            do {
                let resultText = try await service.compare(imageA: imageA, imageB: imageB)
                self.comparisonResult = resultText
            } catch {
    ...
    // Reset function to clear results and return to selection state
    private func reset() {
        comparisonResult = nil
        errorMessage = nil
...
private struct InstructionsView: View {
    var body: some View {
        VStack(spacing: 16) {
            ...
            Text("How to Compare Colors")
                .font(.title2).fontWeight(.bold)
            Text("1. Tap the panels above to select photos of two items.\n2. Tap 'Compare Colors' to get a color analysis.")
                .font(.subheadline)
    ...
    var body: some View {
        VStack(spacing: 16) {
            ...
            Button(action: onReset) {
                Label("Compare New Items", systemImage: "arrow.clockwise")
                    .font(.headline)
                    .padding()
                    .frame(maxWidth: .infinity)
                    .foregroundColor(.white)
